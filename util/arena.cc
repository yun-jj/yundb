...
char* Arena::allocateNewBlock(size_t bytes)
{
  ...
  if (result == nullptr) std::cerr << "Arena allocate fail\n";
...
{
  if (bytes <= 0) std::cerr << "allocate bytes param error\n";
...
char* Arena::allocateAligned(size_t bytes)
{
  ...
  size_t current_mod = reinterpret_cast<uintptr_t>(_available_block) & (align - 1);
